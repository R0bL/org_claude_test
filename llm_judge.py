...
# =============================================================================
# WEB-ENHANCED VALIDATION (with FDA DailyMed + Serper evidence)
# =============================================================================

def format_evidence_for_prompt(evidence: dict) -> str:
    """Format evidence dict into readable prompt context for LLM validation."""
    sections = []
    
    # Drug label info from DailyMed
    if evidence.get("drug_label"):
        label = evidence["drug_label"]
        source = evidence.get("drug_label_source", "")
        
        if source.startswith("dailymed"):
            sections.append(f"""FDA DRUG LABEL (from DailyMed API):
  Labeler: {label.get('labeler', 'N/A')}
  Title: {label.get('title', 'N/A')}
  SPL ID: {label.get('spl_id', 'N/A')}""")
        else:
            # Serper fallback results
            results = label.get("search_results", [])
            if results:
                snippets = [f"  - {r.get('title', 'N/A')}: {r.get('snippet', '')[:100]}" for r in results[:3]]
                sections.append("DRUG LABEL SEARCH RESULTS:\n" + "\n".join(snippets))
    
    # Org search results
    if evidence.get("org_search"):
        org = evidence["org_search"]
        if org.get("source_org_info"):
            snippets = [f"  - {r.get('snippet', '')[:80]}" for r in org["source_org_info"][:2]]
            if snippets:
                sections.append("SOURCE ORG WEB RESULTS:\n" + "\n".join(snippets))
    
    return "\n\n".join(sections) if sections else "No additional evidence found."
...
def parse_llm_json_response(response_text: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # Fallback: try to extract from free-form text
        text_lower = text.lower()
...
def judge_match_with_evidence(
    ...
    """
    Enhanced judge that uses FDA label + web search evidence.
    
    Args:
        row: Prediction row with match details
        dim_org_record: dim_org record being matched to
        evidence: Evidence dict from get_match_evidence()
        client: Anthropic client
        model: Model to use
    ...
    Returns:
        Dict with match, confidence, reason keys
    """
    # Build evidence text from new format
    evidence_sections = []
  ...
  Drug: {evidence.get('fda_drug', 'N/A')}""")
    ...
    # Get the FDA labeler for comparison
    fda_labeler = evidence.get('fda_labeler', 'Unknown')
    # Avoid floats in the prompt
    # Stress Test changes to prompt impact the results (considerations)
    # FAISS
    # Golden DataSet
